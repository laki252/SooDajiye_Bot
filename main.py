...
pyro_app = Client("video_downloader_bot", api_id=API_ID, api_hash=API_HASH, bot_token=TOKEN)
...
async def get_bot_statistics():
    try:
        ...
        return "Statistics currently unavailable."

@pyro_app.on_message(filters.private & filters.command("start"))
...
@pyro_app.on_message(filters.private & filters.command("status"))
async def status_command(client, message: Message):
    stats = await get_bot_statistics()
    await message.reply(stats)

@pyro_app.on_message(filters.private & filters.text)
...
def handle_online(message):
    tele_bot.send_message(message.chat.id, "yes Im alive ✅")
